...
function initSchema() {
  db.serialize(() => {
    ...
    db.run(`UPDATE plans SET name='Professional',price_monthly=79,  max_contacts=50000,  max_campaigns=200, max_emails_per_day=30000,  max_email_accounts=30,  max_ai_credits=1000, features=? WHERE id='plan_pro'`,       [PRO_F]);
    db.run(`UPDATE plans SET name='Agency',      price_monthly=299, max_contacts=200000, max_campaigns=999, max_emails_per_day=100000, max_email_accounts=100, max_ai_credits=5000, features=? WHERE id='plan_unlimited'`, [AGENCY_F]);
  });
}
